...
<section id="container" >
    ...
    <section id="main-content" class="col-xs-10">
        <section class="wrapper">
            <div class="toolkit" style="background-color: #f8f8f8;">
                <g:if test="${unfirst}">
                <span><a href="javascript:history.go(-1);"><i class="fa fa-chevron-left" style="margin-right: 10px;"></i>下属任务</a></span>
                </g:if>
                <g:else><span>下属任务</span></g:else>
            </div>
            <g:if test="${bumenList&&session.user.pid!=3}">
            <div class="index-group mt10 mb0">
                ...
                <ul class="xsreport clearfix">
                    <g:each in="${bumenList}" var="info">
                        <li><g:link action="taskSubordinate" params="[bid: info.id,cid: info.cid]"><span>${info.name}</span></g:link></li>
                    </g:each>
                </ul>
            </div>
            </g:if>
            <g:if test="${companyUserList}">
            <div class="index-group" style="border-top: transparent;">
                <div class="index-head">
