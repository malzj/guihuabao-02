...
(function(window, document, plupload, undef) {
	...
	plupload.runtimes.Flash = plupload.addRuntime("flash", {
		...
		init : function(uploader, callback) {
			...
			browseButton = $(uploader.settings.browse_button).get(0);
			...
			if (uploader.settings.container) {
				...
				//RICKY 修改为jquery参数
				container = $(uploader.settings.container).get(0);
			...
			uploader.bind("Flash:Init", function() {				
				...
				uploader.bind("Refresh", function(up) {
					...
					browseButton = $(uploader.settings.browse_button).get(0);
					if (browseButton) {
						browsePos = plupload.getPos(browseButton, $(up.settings.container).get(0));
	...
	plupload.runtimes.Html5 = plupload.addRuntime("html5", {
		...
		getFeatures : function() {
			...
			return {
				...
				triggerDialog: (plupload.ua.gecko && window.FormData || plupload.ua.webkit || window.FileReader)
			};
		},
		...
		init : function(uploader, callback) {
			...
			uploader.bind("PostInit", function() {
				...
				var dropElm = $(uploader.settings.drop_element).get(0);
				...
				if (dropElm) {
					...
					if (fakeSafariDragDrop) {
						plupload.addEvent(dropElm, 'dragenter', function(e) {
							...
							//RICKY
							dropPos = plupload.getPos(dropElm, $(uploader.settings.container).get(0));
			...
			uploader.bind('Destroy', function(up) {
				...
				if (up.settings.container) {
					...
					//RICKY 修改为jquery对象
					container = $(up.settings.container).get(0);
				}
				
				// Remove mark-up
				if(container && elements.inputContainer){
